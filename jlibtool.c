...
static void usage(int code)
{
	printf("Usage: jlibtool [OPTIONS...] COMMANDS...\n"
	       "jlibtool is a replacement for GNU libtool with similar functionality.\n\n"

	       "    --config          show all configuration variables\n"
	       "                      use --config=FOO to show variable FOO\n"
	       "    --debug           enable verbose shell tracing\n"
	       "    --dry-run         display commands without modifying any files\n"
	       "    --help            display this help message and exit\n"
	       "    --mode=MODE       use operational mode MODE (you *must* set mode)\n"

	       "    --silent          don't print informational messages\n"
	       "    --tag=TAG         Ignored for libtool compatibility\n"
	       "    --version         print version information\n"


	       "    --shared          Build shared libraries when using --mode=link\n"
	       "    --export-all      Try to export 'def' file on some platforms\n"

	       "\nMODE must be one of the following:\n\n"
	       "      compile         compile a source file into a jlibtool object\n"
	       "      execute         automatically set library path, then run a program\n"
               "      install         install libraries or executables\n"
               "      link            create a library or an executable\n"

	       "\nMODE-ARGS can be the following:\n\n"
	       "      -export-dynamic  accepted and ignored\n"
	       "      -module          create a module when linking\n"
	       "      -shared          create a shared library when linking\n"
	       "      -prefer-pic      prefer position-independent-code when compiling\n"
	       "      -prefer-non-pic  prefer non position-independent-code when compiling\n"
	       "      -static          create a static library when linking\n"
	       "      -no-install      link libraries locally\n"
	       "      -rpath arg       Set install path for shared libraries\n"
	       "      -l arg           pass '-l arg' to the link stage\n"
	       "      -L arg           pass '-L arg' to the link stage\n"
	       "      -R dir           add 'dir' to runtime library search path.\n"
	       "      -Zexe            accepted and ignored\n"
	       "      -avoid-version   accepted and ignored\n");
